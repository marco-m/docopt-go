...
func TestCommands(t *testing.T) {
	if v, err := testParser.Parse("Usage: prog add", []string{"add"}, ""); reflect.DeepEqual(v, Opts{"add": true}) != true {
		t.Error(err)
	}
	if v, err := testParser.Parse("Usage: prog [add]", []string{}, ""); reflect.DeepEqual(v, Opts{"add": false}) != true {
		t.Error(err)
	}
	if v, err := testParser.Parse("Usage: prog [add]", []string{"add"}, ""); reflect.DeepEqual(v, Opts{"add": true}) != true {
		t.Error(err)
	}
	if v, err := testParser.Parse("Usage: prog (add|rm)", []string{"add"}, ""); reflect.DeepEqual(v, Opts{"add": true, "rm": false}) != true {
		t.Error(err)
	}
	if v, err := testParser.Parse("Usage: prog (add|rm)", []string{"rm"}, ""); reflect.DeepEqual(v, Opts{"add": false, "rm": true}) != true {
		t.Error(err)
	}
	if v, err := testParser.Parse("Usage: prog a b", []string{"a", "b"}, ""); reflect.DeepEqual(v, Opts{"a": true, "b": true}) != true {
		t.Error(err)
	}
...
func TestBasicPatternMatching(t *testing.T) {
	...
	y = patternList{}
	z := patternList{
		newOption("-a", "", 0, false), newArgument("N", 9),
...
func TestCountMultipleFlags(t *testing.T) {
	if v, err := testParser.Parse("usage: prog [-v]", []string{"-v"}, ""); reflect.DeepEqual(v, Opts{"-v": true}) != true {
		t.Error(err)
	}
	if v, err := testParser.Parse("usage: prog [-vv]", []string{}, ""); reflect.DeepEqual(v, Opts{"-v": 0}) != true {
		t.Error(err)
	}
	if v, err := testParser.Parse("usage: prog [-vv]", []string{"-v"}, ""); reflect.DeepEqual(v, Opts{"-v": 1}) != true {
		t.Error(err)
	}
	if v, err := testParser.Parse("usage: prog [-vv]", []string{"-vv"}, ""); reflect.DeepEqual(v, Opts{"-v": 2}) != true {
		t.Error(err)
	}
	...
	if v, err := testParser.Parse("usage: prog [-v...]", []string{"-vvvvvv"}, ""); reflect.DeepEqual(v, Opts{"-v": 6}) != true {
		t.Error(err)
	}
	if v, err := testParser.Parse("usage: prog [--ver --ver]", []string{"--ver", "--ver"}, ""); reflect.DeepEqual(v, Opts{"--ver": 2}) != true {
		t.Error(err)
	}
}
...
func TestDefaultValueForPositionalArguments(t *testing.T) {
	...
	if v, err := testParser.Parse(doc, []string{}, ""); reflect.DeepEqual(v, Opts{"--data": []string{"x"}}) != true {
		t.Error(err)
	}

	doc = "Usage: prog [--data=<data>...]\nOptions:\n\t-d --data=<arg>    Input data [default: x y]"
	if v, err := testParser.Parse(doc, []string{}, ""); reflect.DeepEqual(v, Opts{"--data": []string{"x", "y"}}) != true {
		t.Error(err)
	}

	doc = "Usage: prog [--data=<data>...]\nOptions:\n\t-d --data=<arg>    Input data [default: x y]"
	if v, err := testParser.Parse(doc, []string{"--data=this"}, ""); reflect.DeepEqual(v, Opts{"--data": []string{"this"}}) != true {
		t.Error(err)
	}
}

func TestIssue59(t *testing.T) {
	if v, err := testParser.Parse("usage: prog --long=<a>", []string{"--long="}, ""); reflect.DeepEqual(v, Opts{"--long": ""}) != true {
		t.Error(err)
	}

	if v, err := testParser.Parse("usage: prog -l <a>\noptions: -l <a>", []string{"-l", ""}, ""); reflect.DeepEqual(v, Opts{"-l": ""}) != true {
		t.Error(err)
	}
}

func TestOptionsFirst(t *testing.T) {
	if v, err := testParser.Parse("usage: prog [--opt] [<args>...]", []string{"--opt", "this", "that"}, ""); reflect.DeepEqual(v, Opts{"--opt": true, "<args>": []string{"this", "that"}}) != true {
		t.Error(err)
	}

	if v, err := testParser.Parse("usage: prog [--opt] [<args>...]", []string{"this", "that", "--opt"}, ""); reflect.DeepEqual(v, Opts{"--opt": true, "<args>": []string{"this", "that"}}) != true {
		t.Error(err)
	}
...
func TestIssue68OptionsShortcutDoesNotIncludeOptionsInUsagePattern(t *testing.T) {
	args, err := testParser.Parse("usage: prog [-ab] [options]\noptions: -x\n -y", []string{"-ax"}, "")
